...
sub delete_overdue_rules {
    ...
    return try {
        ...
        my $item_type       = $body->{rule_set}->{context}->{item_type_id};
        my $branchcode      = $body->{rule_set}->{context}->{library_id};
        my $patron_category = $body->{rule_set}->{context}->{patron_category_id};
        my $trigger_number  = $body->{trigger_number};
        ...
        # TODO: handle trigger number check here?

        Koha::CirculationRules->delete_overdue_rules($body);

        # TODO: Add error handling for errors to do with inexistant trigger number
        # TODO: Add error handling for errors to do with db state not matching what was sent -> return message informing of detected changes

        # TODO: determine what the return value should be and amend accordingly
        # my $return = { map { $_->rule_name => $_->rule_value } @{$new_rules} };
        # $return->{context} =
        #     { library_id => $branchcode, patron_category_id => $patron_category, item_type_id => $item_type };
