...
# 3. Pre-filter items to only those matching known delay values
# 4. Group items by days_overdue for batch processing
# 5. Apply rule resolution with fallback hierarchy
# 6. Queue actions with digest grouping
# 7. Execute grouped digest notices and individual actions
# ```
...
# ### Application Level
# - **Rule caching**: Build complete rule cache with fallback resolution at startup
# - **Calendar caching**: One `Koha::Calendar` object per library, reused across items
# - **Batch processing**: Group items by days_overdue to minimise rule lookups
# - **Pre-filtering**: Only process items that could match existing rule delays

# ### Memory Optimisation
# - Process items in batches if dataset is very large
# - Clear processed items from memory during execution
# - Use array references for large datasets
...
# ### Code Organisation
# ```
# - Main script: process_overdues.pl
